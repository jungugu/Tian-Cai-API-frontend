...
import {
  doDailySignInUsingPOST, emailBandingHandleUsingPOST, emailUnBandingHandleUsingPOST,
...
const UserInfo: React.FC = () => {
  ...
  const updateVoucher = async () => {
    ...
    const res = await RandomSignUsingGET();
  ...
  const steps: TourProps['steps'] = [
    ...
    {
      title: '我的钱包',
      description: <span>这里是您的钱包，糯米用于平台接口的调用费用。<br/>除了充值糯米外，您还可以每日签到或者邀请好友注册来获得糯米</span>,
      target: () => ref2.current,
    }
  ];
  ...
  const updateUserInfo = async () => {
    ...
    if (res.data && res.code === 0) {
      setInitialState({currentUser: res2.data})
  ...
  const props: UploadProps = {
    ...
    action: `http://localhost:8101/api/file/upload?biz=user_avatar`,
  ...
  const handleBindEmailSubmit = async (values: API.UserEmailBindingRequest) => {
    try {
      // 绑定邮箱
      const res = await emailBandingHandleUsingPOST({
        ...values,
      });
      if (res.data && res.code === 0) {
  ...
  const handleUnBindEmailSubmit = async (values: API.UserEmailBindingRequest) => {
    try {
      // 绑定邮箱
      const res = await emailUnBandingHandleUsingPOST({...values});
      if (res.data && res.code === 0) {
        setOpenEmailModal(false)
  ...
  return (
    <Spin spinning={loading}>
      ...
      >
          ...
          extra={
            <>
              <Tooltip title={"用于接收订单信息"}>
                ...
                }>{currentUser?.email ? '更新邮箱' : "绑定邮箱"}</Button>
              </Tooltip>
        ...
        >
          ...
          <Descriptions column={1}>
            ...
            <div>
              ...
              <Paragraph
                copyable={valueLength(currentUser?.invitationCode)}
              >
            ...
            <div>
              <h4>我的id：</h4>
              <Paragraph
                copyable={valueLength(currentUser?.id)}
              >
                {currentUser?.id}
              </Paragraph>
            </div>
            <div>
              <h4>我的邮箱：</h4>
              <Paragraph
                copyable={valueLength(currentUser?.email)}
              >
                {valueLength(currentUser?.email) ? currentUser?.email : '未绑定邮箱'}
              </Paragraph>
            </div>
          </Descriptions>
        </ProCard>
        ...
        >
          ...
          <Button loading={dailyCheckInLoading}
            ...
            const res = await doDailySignInUsingPOST()
            if (res.data && res.code === 0) {
              const res = await getLoginUserUsingGET();
              if (res.data && res.code === 0) {
        ...
        <ProCard
          ...
          extra={
            <Button
              ...
              onClick={updateVoucher}>{(currentUser?.accessKey && currentUser?.secretKey) ? "更新" : "生成"}凭证</Button>
          }
        >
          {
            (currentUser?.accessKey && currentUser?.secretKey) ? (
              <Descriptions column={1}>
                <Descriptions.Item label="AccessKey">
                  <Paragraph copyable={valueLength(currentUser?.accessKey)}>
                    {currentUser?.accessKey}
                  </Paragraph>
                </Descriptions.Item>
                <Descriptions.Item label="SecretKey">
                  <Paragraph copyable={valueLength(currentUser?.secretKey)}>
      ...
      <EmailModal unbindSubmit={handleUnBindEmailSubmit} bindSubmit={handleBindEmailSubmit} data={currentUser}
