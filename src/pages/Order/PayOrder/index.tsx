...
import {
  addProductOrderUsingPOST,
  queryOrderStatusUsingPOST
} from "@/services/tianCaiAPI/productOrderController";
import {getProductInfoByIdUsingGET} from "@/services/tianCaiAPI/productInfoController";
const PayOrder: React.FC = () => {
  const [loading, setLoading] = useState<boolean>(false);
  const [productInfo, setProductInfo] = useState<API.ProductInfo>();
  ...
  const toAlipay = async () => {
    ...
    setLoading(true)
    // @ts-ignore
    const res = await addProductOrderUsingPOST({productId: params.id, payType: "ZFB"})
    if (res.code === 0 && res.data) {
  ...
  useEffect(async () => {
    ...
    const currentURL = location.pathname; // 获取当前路径
    const id = currentURL.split("/")[3];
    const dataProductInfo = await getProductInfoByIdUsingGET({id: id});
    setProductInfo(dataProductInfo?.data);
