...
import {
  ...
  listMyProductOrderByPageUsingPOST
} from "@/services/tianCaiAPI/productOrderController";
import {listMyProductInfoByPageUsingPOST} from "@/services/tianCaiAPI/productInfoController";
...
export default () => {
  ...
  const handleClosed = async (record: API.ProductOrderVO) => {
    ...
    try {
      const res = await closeProductOrderUsingPOST({
  ...
  return (
    <ProTable<API.ProductOrderVO>
      ...
      request={async (params) => {
        setLoading(true)
        const res = await listMyProductOrderByPageUsingPOST({...params});
        if (res.data) {
          setLoading(false)
          return {
